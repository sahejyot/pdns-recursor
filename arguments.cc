...
// list() method removed - not in header declaration
...
gid_t ArgvMap::asGid(const string& arg)
{
  ...
  if (gid == 0 && cptr_ret == cptr_orig) {
    ...
    gid = group->gr_gid;
#else
    throw ArgException("'" + arg + string("' contains invalid group (group name resolution not supported on Windows)"));
#endif
  }
  return gid;
}
...
uid_t ArgvMap::asUid(const string& arg)
{
  ...
  if (uid == 0 && cptr_ret == cptr_orig) {
    ...
    uid = pwent->pw_uid;
#else
    throw ArgException("'" + arg + string("' contains invalid user (user name resolution not supported on Windows)"));
#endif
  }
  return uid;
}
