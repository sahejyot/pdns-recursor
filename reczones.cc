...
// rec-rust-lib/cxxsettings.hh - only needed for reloadZoneConfiguration() and other zone loading functions
// Guarded for minimal setup (primeHints only)
#ifdef ENABLE_ZONE_LOADING
#include "rec-rust-lib/cxxsettings.hh"
#endif
// rec-system-resolve.hh - only needed for convertServersForAD() which uses pdns::fromNameOrIP
// Guarded for minimal setup (primeHints only)
#ifdef ENABLE_ZONE_LOADING
