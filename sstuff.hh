...
public:
  ...
  void recvFrom(string& dgram, ComboAddress& remote) const
  {
    ...
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    auto bytes = recvfrom(d_socket, reinterpret_cast<char*>(dgram.data()), static_cast<int>(dgram.size()), 0, reinterpret_cast<sockaddr*>(&remote), reinterpret_cast<int*>(&remlen));
    if (bytes < 0) {
