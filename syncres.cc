...
{
  std::cout << "[DEBUG] pushResolveIfNotInNegCache: qname=" << qname << " qtype=" << qtype.getCode() << std::endl;
  ...
  if (!inNegCache) {
    ...
    std::cout << "[DEBUG] pushResolveIfNotInNegCache: scheduled task" << std::endl;
  }
  else {
    std::cout << "[DEBUG] pushResolveIfNotInNegCache: in negative cache, skipping" << std::endl;
  }
  return !inNegCache;
}
...
SyncRes::~SyncRes()
{
  std::cout << "[DEBUG] SyncRes destructor: START" << std::endl;
  std::cout.flush();
  
  // Log state of potentially blocking members
  std::cout << "[DEBUG] SyncRes destructor: d_asyncResolve is " << (d_asyncResolve ? "set" : "nullptr") << std::endl;
  std::cout.flush();
  
  std::cout << "[DEBUG] SyncRes destructor: About to destroy d_asyncResolve (std::function)" << std::endl;
  std::cout.flush();
  // d_asyncResolve will be destroyed here (std::function destructor)
  // If this blocks, we won't see the next log
  
  std::cout << "[DEBUG] SyncRes destructor: d_asyncResolve destroyed" << std::endl;
  std::cout.flush();
  
  std::cout << "[DEBUG] SyncRes destructor: About to destroy d_pdl (shared_ptr)" << std::endl;
  std::cout.flush();
  // d_pdl will be destroyed here (shared_ptr destructor)
  
  std::cout << "[DEBUG] SyncRes destructor: d_pdl destroyed" << std::endl;
  std::cout.flush();
  
  std::cout << "[DEBUG] SyncRes destructor: About to destroy d_outgoingProtobufServers" << std::endl;
  std::cout.flush();
  // d_outgoingProtobufServers will be destroyed here
  
  std::cout << "[DEBUG] SyncRes destructor: d_outgoingProtobufServers destroyed" << std::endl;
  std::cout.flush();
  
  std::cout << "[DEBUG] SyncRes destructor: About to destroy d_frameStreamServers" << std::endl;
  std::cout.flush();
  // d_frameStreamServers will be destroyed here
  
  std::cout << "[DEBUG] SyncRes destructor: d_frameStreamServers destroyed" << std::endl;
  std::cout.flush();
  
  std::cout << "[DEBUG] SyncRes destructor: About to destroy d_validationContext" << std::endl;
  std::cout.flush();
  // d_validationContext will be destroyed here
  
  std::cout << "[DEBUG] SyncRes destructor: d_validationContext destroyed" << std::endl;
  std::cout.flush();
  
  std::cout << "[DEBUG] SyncRes destructor: About to destroy d_trace (ostringstream)" << std::endl;
  std::cout.flush();
  // d_trace will be destroyed here (ostringstream destructor)
  
  std::cout << "[DEBUG] SyncRes destructor: d_trace destroyed" << std::endl;
  std::cout.flush();
  
  std::cout << "[DEBUG] SyncRes destructor: END (all members destroyed)" << std::endl;
  std::cout.flush();
}
...
{
  std::cout << "[DEBUG] SyncRes::beginResolve entry qname=" << qname << " qtype=" << qtype.getCode() << " depth=" << depth << std::endl;
  std::cout << "[DEBUG] SyncRes flags: doIPv4=" << s_doIPv4 << " doIPv6=" << s_doIPv6 << " qnamemin=" << s_qnameminimization << " noEDNS=" << s_noEDNS << std::endl;
  ...
  if (doSpecialNamesResolve(qname, qtype, qclass, ret)) {
    std::cout << "[DEBUG] beginResolve: doSpecialNamesResolve handled name, returning 0" << std::endl;
  ...
  if (isUnsupported(qtype)) {
    std::cout << "[DEBUG] beginResolve: unsupported qtype=" << qtype.getCode() << ", returning -1" << std::endl;
    return -1;
  }
  ...
  if (qclass == QClass::ANY) {
    ...
    std::cout << "[DEBUG] beginResolve: unsupported qclass=" << static_cast<unsigned>(qclass) << ", returning -1" << std::endl;
    return -1;
  }
  ...
  set<GetBestNSAnswer> beenthere;
  Context context;
  std::cout << "[DEBUG] SyncRes::doResolve starting" << std::endl;
  int res = doResolve(qname, qtype, ret, depth, beenthere, context);
  std::cout << "[DEBUG] SyncRes::doResolve returned res=" << res << " retCount=" << ret.size() << std::endl;
  std::cout << "[DEBUG] SyncRes::beginResolve exit res=" << res << " records=" << ret.size() << std::endl;
...
const char* isoDateTimeMillis(const struct timeval& tval, timebuf_t& buf)
{
  ...
  struct tm tmval{};
  #ifdef _WIN32
  time_t _tt = static_cast<time_t>(tval.tv_sec);
  localtime_s(&tmval, &_tt);
  #endif
  size_t len = strftime(buf.data(), buf.size(), s_timestampFormat.c_str(),
  #ifdef _WIN32
                        &tmval
  #else
...
static const char* timestamp(time_t arg, timebuf_t& buf)
{
  ...
  struct tm tmval{};
  #ifdef _WIN32
  localtime_s(&tmval, &arg);
  #endif
  size_t len = strftime(buf.data(), buf.size(), s_timestampFormat.c_str(),
  #ifdef _WIN32
                        &tmval
  #else
                        localtime_r(&arg, &tmval)
  #endif
  );
  if (len == 0) {
...
{
  std::cout << "[DEBUG] asyncresolveWrapper: called for " << address.toStringWithPort() << " qname=" << domain << " qtype=" << type << " d_asyncResolve=" << (d_asyncResolve ? "set" : "null") << std::endl;
  /* what is your QUEST?
  ...
  */
  ...
  for (int tries = 0; tries < 2; ++tries) {
    ...
    if (d_asyncResolve) {
      std::cout << "[DEBUG] doResolveAtThisIP: invoking d_asyncResolve to " << address.toStringWithPort() << " qname=" << sendQname << " qtype=" << type << " EDNSLevel=" << EDNSLevel << std::endl;
      ret = d_asyncResolve(address, sendQname, type, doTCP, sendRDQuery, EDNSLevel, now, srcmask, ctx, res, chained);
      std::cout << "[DEBUG] doResolveAtThisIP: d_asyncResolve returned ret=" << static_cast<int>(ret) << std::endl;
    }
  else {
      std::cout << "[DEBUG] doResolveAtThisIP: calling global asyncresolve (d_asyncResolve is null)" << std::endl;
      #if defined(HAVE_LUA) && HAVE_LUA
      ...
      #else
      std::set<uint16_t> emptyTypes;
      ret = asyncresolve(address, sendQname, type, doTCP, sendRDQuery, EDNSLevel, now, srcmask, ctx, d_outgoingProtobufServers, d_frameStreamServers, emptyTypes, res, chained);
      #endif
      std::cout << "[DEBUG] doResolveAtThisIP: global asyncresolve returned ret=" << static_cast<int>(ret) << std::endl;
    }
...
{
  std::cout << "[DEBUG] doResolve: qname=" << qname << " qtype=" << qtype.getCode() << " depth=" << depth << std::endl;
  ...
  const auto fwiter = getBestAuthZone(&fwdomain);
  const bool forwarded = (t_sstorage.domainmap && !t_sstorage.domainmap->empty() && fwiter != t_sstorage.domainmap->end());
  if (forwarded) {
  ...
  for (unsigned int i = 0; i <= qnamelen; i++) {
    ...
    for (int tries = 0; tries < 2 && bestns.empty(); ++tries) {
      ...
      std::cout << "[DEBUG] doResolve: calling getBestNSFromCache for nsdomain=" << nsdomain << std::endl;
      ...
      std::cout << "[DEBUG] doResolve: bestns size after getBestNSFromCache=" << bestns.size() << std::endl;
      if (forwarded) {
        break;
      }
    }
...
int SyncRes::doResolveNoQNameMinimization(const DNSName& qname, const QType qtype, vector<DNSRecord>& ret, unsigned int depth, set<GetBestNSAnswer>& beenthere, Context& context, bool* fromCache, StopAtDelegation* stopAtDelegation) // NOLINT(readability-function-cognitive-complexity)
{
  ...
  if (!d_updatingRootNS || qtype.getCode() != QType::NS || !qname.isRoot()) {
    ...
    std::cout << "[DEBUG] doResolveNoQNameMinimization: calling doResolveAt nsset.size=" << nsset.size() << " subdomain=" << subdomain << " qname=" << qname << " qtype=" << qtype.getCode() << std::endl;
...
vector<ComboAddress> SyncRes::getAddrs(const DNSName& qname, unsigned int depth, const string& prefix, set<GetBestNSAnswer>& beenthere, bool cacheOnly, unsigned int& addressQueriesForNS)
{
  ...
  try {
    ...
    // Note: Glue records are cached with from=boost::none, so they should be globally accessible
    // The cache matching logic should handle entries with from=boost::none regardless of who parameter
    res_t cset;
    int cacheResultA = -1;
    if (s_doIPv4) {
      cacheResultA = g_recCache->get(d_now.tv_sec, qname, QType::A, flags, &cset, d_cacheRemote, d_routingTag);
      if (cacheResultA > 0) {
      collectAddresses<ARecordContent>(cset, ret);
    }
    }
    int cacheResultAAAA = -1;
    if (s_doIPv6) {
      cacheResultAAAA = g_recCache->get(d_now.tv_sec, qname, QType::AAAA, flags, &cset, d_cacheRemote, d_routingTag);
      if (cacheResultAAAA > 0) {
      if (collectAddresses<AAAARecordContent>(cset, ret)) {
        seenV6 = true;
      }
      }
    }
    // Debug: Check if glue is in cache for gtld servers
    if (qname.toString().find("gtld-servers.net") != std::string::npos) {
      std::cout << "[DEBUG] getAddrs: Looking for " << qname << " A=" << cacheResultA << " AAAA=" << cacheResultAAAA << " found=" << ret.size() << " addresses d_cacheRemote=" << d_cacheRemote.toString() << " d_routingTag=" << (d_routingTag ? d_routingTag->c_str() : "none") << std::endl;
    }
    if (ret.empty()) {
...
{
  std::cout << "[DEBUG] getBestNSFromCache: qname=" << qname << " qtype=" << qtype.getCode() << " depth=" << depth << std::endl;
  DNSName subdomain(qname);
  bestns.clear();
  ...
  do {
    ...
    *flawedNSSet = false;
    ...
    bool isAuth = false;
    std::cout << "[DEBUG] getBestNSFromCache: before get(), d_cacheRemote=" << d_cacheRemote.toString() << ", d_routingTag=" << (d_routingTag ? d_routingTag->c_str() : "none") << ", subdomain=" << subdomain << ", subdomain.isRoot()=" << subdomain.isRoot() << std::endl;
    int cacheResult = g_recCache->get(d_now.tv_sec, subdomain, QType::NS, flags, &nsVector, d_cacheRemote, d_routingTag, nullptr, nullptr, nullptr, nullptr, &isAuth);
    std::cout << "[DEBUG] getBestNSFromCache: cache lookup for " << subdomain << "|NS returned " << cacheResult << ", nsVector.size()=" << nsVector.size() << std::endl;
    if (cacheResult > 0) {
      ...
      for (const auto& nsRecord : nsVector) {
        if (nsRecord.d_ttl > (unsigned int)d_now.tv_sec) {
          ...
          int cacheResult = -1;
          if (nrr) {
            if (!nrr->getNS().isPartOf(subdomain)) {
              cacheResult = 1; // Non-glue, always valid
            } else {
              cacheResult = g_recCache->get(d_now.tv_sec, nrr->getNS(), nsqt, flags, doLog() ? &aset : nullptr, d_cacheRemote, d_routingTag);
            }
          }
          if (nrr && cacheResult > 0) {
          ...
          if (qname == DNSName("amazon.com") && nrr) {
            std::cout << "[DEBUG] getBestNSFromCache: NS " << nrr->getNS() << " isPartOf(" << subdomain << ")=" << nrr->getNS().isPartOf(subdomain) << " cacheResult=" << cacheResult << " aset.size()=" << aset.size() << std::endl;
          }
          else if (nrr != nullptr) {
            *flawedNSSet = true;
    ...
    std::cout << "[DEBUG] getBestNSFromCache: No NS found, subdomain.isRoot()=" << subdomain.isRoot() << ", brokeloop=" << brokeloop << std::endl;

    if (subdomain.isRoot() && !brokeloop) {
      // We lost the root NS records
      std::cout << "[DEBUG] getBestNSFromCache: Calling primeHints() to reprime root NS" << std::endl;
      primeHints();
      LOG(prefix << qname << ": Reprimed the root" << endl);
      std::cout << "[DEBUG] getBestNSFromCache: primeHints() completed, calling getRootNS()" << std::endl;
      /* let's prevent an infinite loop */
      if (!d_updatingRootNS) {
        auto log = d_slog ? d_slog->withName("housekeeping") : nullptr;
...
{
  if (!t_sstorage.domainmap || t_sstorage.domainmap->empty()) {
    static SyncRes::domainmap_t emptyMap;
    return emptyMap.end();
  }
  ...
  if (!tns->first.empty()) {
    ...
    try {
      if (qname == DNSName("amazon.com")) {
        std::cout << "[DEBUG] retrieveAddressesForNS: Getting addresses for NS " << tns->first << " cacheOnly=" << cacheOnly << std::endl;
      }
      result = getAddrs(tns->first, depth, prefix, beenthere, cacheOnly, nretrieveAddressesForNS);
      if (qname == DNSName("amazon.com")) {
        std::cout << "[DEBUG] retrieveAddressesForNS: Got " << result.size() << " addresses for NS " << tns->first << std::endl;
      }
    }
...
vState SyncRes::getTA(const DNSName& zone, dsset_t& dsSet, const string& prefix)
{
  ...
  #else
  (void)zone; (void)dsSet; (void)prefix;
  return vState::Insecure;
  #endif
  ...
  for (const auto& record : dsrecords) {
    if (record.d_type == QType::DS) {
      ...
      if (dscontent && isSupportedDS(*dscontent, LogObject(prefix))) {
        ...
        #else
        if (dscontent->d_digesttype > bestDigestType) {
          bestDigestType = dscontent->d_digesttype;
        }
        #endif
...
RCode::rcodes_ SyncRes::updateCacheFromRecords(unsigned int depth, const string& prefix, LWResult& lwr, const DNSName& qname, const QType qtype, const DNSName& auth, bool wasForwarded, const boost::optional<Netmask>& ednsmask, vState& state, bool& needWildcardProof, bool& gatherWildcardProof, unsigned int& wildcardLabelsCount, bool rdQuery, const ComboAddress& remoteIP) // NOLINT(readability-function-cognitive-complexity)
{
  ...
  for (auto& rec : lwr.d_records) {
    ...
    // Focused debug: when resolving amazon.com, print all NS in AUTHORITY with owner and target
    if (qname == DNSName("amazon.com") && rec.d_place == DNSResourceRecord::AUTHORITY && rec.d_type == QType::NS) {
      if (auto rc = getRR<NSRecordContent>(rec)) {
        std::cout << "[DEBUG] amazon.com AUTHORITY NS owner=" << rec.d_name << " target=" << rc->getNS() << std::endl;
      } else {
        std::cout << "[DEBUG] amazon.com AUTHORITY NS owner=" << rec.d_name << " target=(unparsed)" << std::endl;
      }
    }
  ...
  for (auto tCacheEntry = tcache.begin(); tCacheEntry != tcache.end(); ++tCacheEntry) {
    ...
    */
    bool willCache = (tCacheEntry->first.type != QType::NSEC3 && (tCacheEntry->first.type == QType::DS || tCacheEntry->first.type == QType::NS || tCacheEntry->first.type == QType::A || tCacheEntry->first.type == QType::AAAA || isAA || wasForwardRecurse));
    if (qname == DNSName("amazon.com") && (tCacheEntry->first.type == QType::A || tCacheEntry->first.type == QType::AAAA) && tCacheEntry->first.place == DNSResourceRecord::ADDITIONAL) {
      std::cout << "[DEBUG] updateCacheFromRecords: Will cache ADDITIONAL " << (tCacheEntry->first.type == QType::A ? "A" : "AAAA") << " for " << tCacheEntry->first.name << "? " << willCache << " (isAA=" << isAA << " wasForwardRecurse=" << wasForwardRecurse << ") d_cacheRemote=" << d_cacheRemote.toString() << std::endl;
    }
    if (willCache) {
      ...
      if (doCache) {
        ...
        if (tCacheEntry->first.type == QType::NS) {
          std::cout << "[DEBUG] updateCacheFromRecords: caching NS set for name=" << tCacheEntry->first.name << " count=" << tCacheEntry->second.records.size() << " isAA=" << isAA << " auth=" << auth << std::endl;
        }
        if (qname == DNSName("amazon.com") && (tCacheEntry->first.type == QType::A || tCacheEntry->first.type == QType::AAAA) && tCacheEntry->first.place == DNSResourceRecord::ADDITIONAL) {
          std::cout << "[DEBUG] updateCacheFromRecords: About to cache glue " << (tCacheEntry->first.type == QType::A ? "A" : "AAAA") << " for " << tCacheEntry->first.name << " ttl_time=" << tCacheEntry->second.d_ttl_time << " now=" << d_now.tv_sec << " records.size()=" << tCacheEntry->second.records.size();
          if (!tCacheEntry->second.records.empty()) {
            std::cout << " first_record.d_ttl=" << tCacheEntry->second.records[0].d_ttl;
          }
          std::cout << std::endl;
        }
        // CRITICAL FIX: For glue records (A/AAAA in ADDITIONAL), use boost::none for 'from' parameter
        // so they are globally accessible regardless of query source, similar to root hints
        // This ensures glue records cached from one query are accessible during glue resolution
        boost::optional<ComboAddress> cacheFrom = remoteIP;
        boost::optional<Netmask> cacheEdnsmask = (tCacheEntry->first.place == DNSResourceRecord::ANSWER ? ednsmask : boost::none);
        MemRecursorCache::OptTag cacheRoutingTag = d_routingTag;
        if (tCacheEntry->first.place == DNSResourceRecord::ADDITIONAL && (tCacheEntry->first.type == QType::A || tCacheEntry->first.type == QType::AAAA)) {
          cacheFrom = boost::none;
          // Also ensure routingTag is boost::none for glue to match lookups
          cacheRoutingTag = boost::none;
          // CRITICAL: Ensure glue records have minimum TTL to prevent immediate expiration
          // If TTL is 0 or very small, set it to a reasonable minimum (e.g., 60 seconds)
          for (auto& record : tCacheEntry->second.records) {
            if (record.d_ttl <= d_now.tv_sec) {
              record.d_ttl = d_now.tv_sec + 60; // Minimum 60 seconds TTL
              if (qname == DNSName("amazon.com")) {
                std::cout << "[DEBUG] updateCacheFromRecords: Adjusted glue " << (tCacheEntry->first.type == QType::A ? "A" : "AAAA") << " TTL for " << tCacheEntry->first.name << " from " << (record.d_ttl - 60) << " to " << record.d_ttl << std::endl;
              }
            }
          }
          if (qname == DNSName("amazon.com")) {
            std::cout << "[DEBUG] updateCacheFromRecords: Caching glue " << (tCacheEntry->first.type == QType::A ? "A" : "AAAA") << " for " << tCacheEntry->first.name << " with from=boost::none routingTag=boost::none (was from=" << remoteIP.toString() << " routingTag=" << (d_routingTag ? d_routingTag->c_str() : "none") << ")" << std::endl;
          }
        }
        // For glue records, ensure they are always cached (force auth=true to bypass shouldReplace() checks)
        bool forceCacheAuth = (tCacheEntry->first.place == DNSResourceRecord::ADDITIONAL && (tCacheEntry->first.type == QType::A || tCacheEntry->first.type == QType::AAAA));
        bool cacheAuth = forceCacheAuth ? true : (tCacheEntry->first.type == QType::DS ? true : isAA);
        // CRITICAL: Force refresh=true for glue records to ensure shouldReplace() always returns true
        bool cacheRefresh = forceCacheAuth ? true : d_refresh;
        g_recCache->replace(d_now.tv_sec, tCacheEntry->first.name, tCacheEntry->first.type, tCacheEntry->second.records, tCacheEntry->second.signatures, thisRRNeedsWildcardProof ? authorityRecs : *MemRecursorCache::s_emptyAuthRecs, cacheAuth, auth, cacheEdnsmask, cacheRoutingTag, recordState, cacheFrom, cacheRefresh, tCacheEntry->second.d_ttl_time);
        if (qname == DNSName("amazon.com") && (tCacheEntry->first.type == QType::A || tCacheEntry->first.type == QType::AAAA) && tCacheEntry->first.place == DNSResourceRecord::ADDITIONAL) {
          std::cout << "[DEBUG] updateCacheFromRecords: Cached glue with auth=" << cacheAuth << " refresh=" << cacheRefresh << " (was isAA=" << isAA << " d_refresh=" << d_refresh << ")" << std::endl;
        }
...
bool SyncRes::processRecords(const std::string& prefix, const DNSName& qname, const QType qtype, const DNSName& auth, LWResult& lwr, const bool sendRDQuery, vector<DNSRecord>& ret, set<DNSName>& nsset, DNSName& newtarget, DNSName& newauth, bool& realreferral, bool& negindic, vState& state, const bool needWildcardProof, const bool gatherWildcardProof, const unsigned int wildcardLabelsCount, int& rcode, bool& negIndicHasSignatures, unsigned int depth) // // NOLINT(readability-function-cognitive-complexity)
{
  ...
  for (auto& rec : lwr.d_records) {
    ...
    // for ANY answers we *must* have an authoritative answer, unless we are forwarding recursively
      ...
      // We added isDelegation check to handle cases where rec.d_name == qname,
      // but this might cause issues if moreSpecificThan() check fails.
      // Reverting to upstream behavior: only process if moreSpecificThan() is true.
      // bool isDelegation = (rec.d_name == qname);
      // bool isMoreSpecific = moreSpecificThan(rec.d_name, auth);
      // if (isMoreSpecific || isDelegation) {
      if (moreSpecificThan(rec.d_name, auth)) {
        newauth = rec.d_name;
        LOG(prefix << qname << ": Got NS record '" << rec.d_name << "' -> '" << rec.getContent()->getZoneRepresentation() << "'" << endl);
        // Debug: Log NS record processing
        std::cout << "[DEBUG] processRecords: PROCESSING NS record - qname=" << qname << " NS owner=" << rec.d_name << " NS target=" << rec.getContent()->getZoneRepresentation() << " newauth=" << newauth << std::endl;
        ...
        // Debug: Log NS record that was NOT processed as delegation
        std::cout << "[DEBUG] processRecords: NOT processing NS record as delegation - qname=" << qname << " rec.d_name=" << rec.d_name << " auth=" << auth << " (not moreSpecificThan)" << std::endl;
        if (auto content = getRR<NSRecordContent>(rec)) {
          nsset.insert(content->getNS());
        }
      }
...
{
  auto log = d_slog ? d_slog->withName("taskq") : nullptr;

  auto logHelper1 = [&log](const string& ename) {
    if (log) log->info(Logr::Debug, "Failed to probe DoT records, got an exception");
  };
  auto logHelper2 = [&log](const string& msg, const string& ename) {
    if (log) log->error(Logr::Debug, msg);
  };
  LWResult lwr;
  bool truncated{};
  bool spoofed{};
...
{
  std::cout << "[DEBUG] updateQueryCounts: entry" << std::endl;
  try {
    std::cout << "[DEBUG] updateQueryCounts: about to access outqueries counter" << std::endl;
    t_Counters.at(rec::Counter::outqueries)++;
    std::cout << "[DEBUG] updateQueryCounts: outqueries counter accessed" << std::endl;
  } catch (const std::exception& e) {
    std::cout << "[DEBUG] updateQueryCounts: exception accessing outqueries counter: " << e.what() << std::endl;
    throw;
  } catch (...) {
    std::cout << "[DEBUG] updateQueryCounts: unknown exception accessing outqueries counter" << std::endl;
    throw;
  }
  std::cout << "[DEBUG] updateQueryCounts: about to increment d_outqueries" << std::endl;
  d_outqueries++;
  std::cout << "[DEBUG] updateQueryCounts: about to call checkMaxQperQ" << std::endl;
  try {
    checkMaxQperQ(qname);
    std::cout << "[DEBUG] updateQueryCounts: checkMaxQperQ returned" << std::endl;
  } catch (const ImmediateServFailException& e) {
    std::cout << "[DEBUG] updateQueryCounts: ImmediateServFailException from checkMaxQperQ: " << e.reason << std::endl;
    throw;
  } catch (const std::exception& e) {
    std::cout << "[DEBUG] updateQueryCounts: std::exception from checkMaxQperQ: " << e.what() << std::endl;
    throw;
  } catch (...) {
    std::cout << "[DEBUG] updateQueryCounts: unknown exception from checkMaxQperQ" << std::endl;
    throw;
  }
  if (address.sin4.sin_family == AF_INET6) {
    std::cout << "[DEBUG] updateQueryCounts: IPv6, accessing ipv6queries counter" << std::endl;
    t_Counters.at(rec::Counter::ipv6queries)++;
  }
  if (doTCP) {
    if (doDoT) {
      LOG(prefix << qname << ": Using DoT with " << address.toStringWithPort() << endl);
      std::cout << "[DEBUG] updateQueryCounts: DoT, accessing dotoutqueries counter" << std::endl;
...
{
  std::cout << "[DEBUG] doResolveAtThisIP: target=" << remoteIP.toStringWithPort() << " nsName=" << nsName << " qname=" << qname << " qtype=" << qtype.getCode() << " doTCP=" << doTCP << " doDoT=" << doDoT << std::endl;
  std::cout << "[DEBUG] doResolveAtThisIP: calling checkTotalTime" << std::endl;
  checkTotalTime(qname, qtype, extendedError);
  std::cout << "[DEBUG] doResolveAtThisIP: checkTotalTime returned" << std::endl;

  bool chained = false;
  LWResult::Result resolveret = LWResult::Result::Success;
  int preOutQueryRet = RCode::NoError;
  std::cout << "[DEBUG] doResolveAtThisIP: about to check Lua preoutquery" << std::endl;
  ...
  #endif
  {
    std::cout << "[DEBUG] doResolveAtThisIP: entering non-Lua path" << std::endl;
    ednsmask = getEDNSSubnetMask(qname, remoteIP);
    std::cout << "[DEBUG] doResolveAtThisIP: getEDNSSubnetMask returned" << std::endl;
    if (ednsmask) {
      LOG(prefix << qname << ": Adding EDNS Client Subnet Mask " << ednsmask->toString() << " to query" << endl);
      s_ecsqueries++;
    }
    std::cout << "[DEBUG] doResolveAtThisIP: about to call d_eventTrace.add" << std::endl;
    auto match = d_eventTrace.add(RecEventTrace::AuthRequest, qname.toLogString() + '/' + qtype.toString(), true, 0);
    std::cout << "[DEBUG] doResolveAtThisIP: d_eventTrace.add returned" << std::endl;
    std::cout << "[DEBUG] doResolveAtThisIP: about to call updateQueryCounts" << std::endl;
    try {
      updateQueryCounts(prefix, qname, remoteIP, doTCP, doDoT);
      std::cout << "[DEBUG] doResolveAtThisIP: updateQueryCounts returned" << std::endl;
    } catch (const std::exception& e) {
      std::cout << "[DEBUG] doResolveAtThisIP: updateQueryCounts exception: " << e.what() << std::endl;
      throw;
    } catch (...) {
      std::cout << "[DEBUG] doResolveAtThisIP: updateQueryCounts unknown exception" << std::endl;
      throw;
    }
    std::cout << "[DEBUG] doResolveAtThisIP: about to call asyncresolveWrapper" << std::endl;
...
bool SyncRes::processAnswer(unsigned int depth, const string& prefix, LWResult& lwr, const DNSName& qname, const QType qtype, DNSName& auth, bool wasForwarded, const boost::optional<Netmask>& ednsmask, bool sendRDQuery, NsSet& nameservers, std::vector<DNSRecord>& ret, const DNSFilterEngine& dfe, bool* gotNewServers, int* rcode, vState& state, const ComboAddress& remoteIP)
{
  ...
  *rcode = updateCacheFromRecords(depth, prefix, lwr, qname, qtype, auth, wasForwarded, ednsmask, state, needWildcardProof, gatherWildcardProof, wildcardLabelsCount, sendRDQuery, remoteIP);
  ...
  std::cout << "[DEBUG] processAnswer: processing " << lwr.d_records.size() << " records for qname=" << qname << " qtype=" << qtype << " auth=" << auth << std::endl;
  if (qname == DNSName("amazon.com")) {
    std::cout << "[DEBUG] amazon.com processAnswer: Record breakdown:" << std::endl;
    int answerCount = 0, authorityCount = 0, additionalCount = 0;
    for (const auto& rec : lwr.d_records) {
      if (rec.d_place == DNSResourceRecord::ANSWER) answerCount++;
      else if (rec.d_place == DNSResourceRecord::AUTHORITY) authorityCount++;
      else if (rec.d_place == DNSResourceRecord::ADDITIONAL) additionalCount++;
    }
    std::cout << "[DEBUG] amazon.com processAnswer: ANSWER=" << answerCount << " AUTHORITY=" << authorityCount << " ADDITIONAL=" << additionalCount << std::endl;
    for (const auto& rec : lwr.d_records) {
      if (rec.d_place == DNSResourceRecord::ANSWER && rec.d_type == QType::A) {
        std::cout << "[DEBUG] amazon.com processAnswer: ANSWER A record - name=" << rec.d_name << " type=" << rec.d_type << " content=" << rec.getContent()->getZoneRepresentation() << std::endl;
      }
    }
  }
  ...
  if (realreferral) {
    ...
    std::cout << "[DEBUG] processAnswer: realreferral=true, got " << nsset.size() << " NS records, newauth=" << newauth << std::endl;
                         ...
                         map<DNSName, vector<ComboAddress>>* fallBack)
{
  ...
  for (;;) { // we may get more specific nameservers
    if (qname == DNSName("amazon.com")) {
      std::cout << "[DEBUG] doResolveAt: Loop restart, auth=" << auth << " nameservers.size()=" << nameservers.size() << std::endl;
    }
    ...
    for (auto tns = rnameservers.cbegin();; ++tns) {
      ...
      if (tns->first.empty() && !wasForwarded) {
        ...
        #else
        DNSFilterEngine emptyDfe;
        bool done = processAnswer(depth, prefix, lwr, qname, qtype, auth, false, ednsmask, sendRDQuery, nameservers, ret, emptyDfe, &gotNewServers, &rcode, context.state, s_oobRemote);
        #endif
        if (done) {
          return rcode;
        }
        if (gotNewServers) {
          if (qname == DNSName("amazon.com")) {
            std::cout << "[DEBUG] doResolveAt: gotNewServers=true, auth=" << auth << " nameservers.size()=" << nameservers.size() << std::endl;
          }
        ...
        for (remoteIP = remoteIPs.begin(); remoteIP != remoteIPs.end(); ++remoteIP) {
          ...
          if (qname == DNSName("amazon.com") || qname == DNSName("google.com")) {
            std::cout << "[DEBUG] doResolveAt: Querying " << remoteIP->toStringWithPort() << " for " << qname << " qtype=" << qtype << " auth=" << auth << " nsName=" << tns->first << std::endl;
          }
          ...
          */
          ...
          #else
          DNSFilterEngine emptyDfe2;
          bool done = processAnswer(depth, prefix, lwr, qname, qtype, auth, wasForwarded, ednsmask, sendRDQuery, nameservers, ret, emptyDfe2, &gotNewServers, &rcode, context.state, *remoteIP);
          #endif
          if (done) {
            return rcode;
          }
...
int directResolve(const DNSName& qname, const QType qtype, const QClass qclass, vector<DNSRecord>& ret, const shared_ptr<RecursorLua4>& pdl, bool qnamemin, Logr::log_t slog)
{
  ...
  try {
    ...
    SLOG(g_log << Logger::Warning << "Failed to resolve " << qname << ", got pdns exception: " << e.reason << endl,
         log->error(Logr::Warning, e.reason, msg));
    ret.clear();
  }
  catch (const ImmediateServFailException& e) {
