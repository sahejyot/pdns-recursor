...
size_t MemRecursorCache::getRecordSets(size_t perShard, size_t maxSize, std::string& ret)
{
  ...
  for (auto& shard : d_maps) {
    ...
    for (auto recordSet = sidx.rbegin(); recordSet != sidx.rend(); ++recordSet) {
      ...
      if (ret.size() > maxSize) {
        message.rollback();
        log->info(Logr::Info, "Produced cache dump (max size reached)");
        return count;
      }
      ++count;
      ++thisShardCount;
  ...
  log->info(Logr::Info, "Produced cache dump");
  return count;
}
...
size_t MemRecursorCache::putRecordSets(const std::string& pbuf)
{
  ...
  try {
    ...
    log->info(Logr::Info, "Processed cache dump");
    return inserted;
  }
